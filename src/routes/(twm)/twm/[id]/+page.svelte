...
>
    <div class="w-[640px] h-[480px] relative">
        <div class="absolute z-2 top-[0px] w-full flex flex-col items-center">
            <img
                alt="The World Machine's screen"
                src={BaseTV}
                class="no-antialias w-[280px]"
            />
        </div>
        <div class="absolute z-1 top-[0px] w-full flex flex-col items-center">
            <img
                alt="static"
                src={Static3}
                class="no-antialias w-2xs opacity-75 h-[248px]"
            />
        </div>
        <div class="absolute z-0 top-[140px] w-full flex flex-col items-center">
            <img
                alt="The Entity (TWM)"
                src={Entity}
                class="no-antialias w-4xs brightness-300"
            />
        </div>
        <div class="z-2 absolute top-[270px] w-full flex flex-col items-center">
            <img
                alt="Dialogue Balloon"
                src={Dialogue}
                class="absolute top-[32px] no-antialias"
                bind:clientWidth={ballon_w}
                bind:clientHeight={ballon_h}
            />
            <div class="absolute z-3">
                <p class="text-left" style="min-width: {ballon_w}px;">
                    {apiData.name}
                </p>
            </div>
            ...
            >
                ...
                >
                <div
                    class="w-[25%] h-[100%] max-h-[100%] flex justify-end items-center"
                >
                    <img
                        class="relative no-antialias w-auto h-full"
                        style=""
                        alt="Nikosona"
                        src={apiData.img_link}
                    />
                </div>
            </div>
            <div class="absolute z-5 top-[160px]">
                <p class="text-left" style="min-width: {ballon_w}px;">
                    Created by {apiData.author}
                </p>
            </div>
        </div>
    </div>
    <div class="fixed top-2 left-2">
        <a class="btn" href={$beforePage}>Go Back</a>
    </div>
</div>
