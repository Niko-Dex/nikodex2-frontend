<script lang="ts">
  ...
  async function acceptSubmission(submission: Submission) {
    const createFetch = fetch(`/api/data/user/userid?id=${submission.user_id}`)
      .then(async (r) => {
        const jsonData = await r.json();
          ...
          >
            ...
            <td class="px-3 py-2">
              ...
              <button
                ...
                onclick={async () => {
                  if (
                    ...
                    await removeSubmission(currentSubmission.id);
                  }
                }}>[Delete]</button
              >
              /
              <button
                ...
                onclick={async () => await acceptSubmission(currentSubmission)}
                >[Accept]</button
              >
            </td>
          </tr>
