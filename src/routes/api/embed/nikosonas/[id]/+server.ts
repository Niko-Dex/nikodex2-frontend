...
function printText(ctx: CanvasRenderingContext2D, img: Image, text: string, x: number, y: number) {
...
async function postProcessing(fetch: (s: string) => Promise<Response>, canvas: Canvas, id: string) {
...
export async function GET({ fetch, params }) {
    try {
        ...
        printText(ctx, inputImg, textId, metadata.location.id[0], metadata.location.id[1]);
        ...
        if (d_creator.length > metadata.maxlen * maxLineCreator - 2) {
            // - 2 for ellipsis
            d_creator = d_creator.slice(0, metadata.maxlen * maxLineCreator - 2 - 3) + "...";
        ...
        for (let i = 0; i < crcStr.length; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                if ((d & (1 << j)) > 0) {
                    ctx.fillStyle = "white";
                    ctx.fillRect(
                        metadata.location.barcode[0] + i * metadata.size.barcode.bar[0] + j + 1, // always one empty line at the start of every block
