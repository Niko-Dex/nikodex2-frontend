    ...
    function drawBoard() {
        ...
        for (let i = 0; i < gameState.size[0]; i++) {
            for (let j = 0; j < gameState.size[1]; j++) {
                ...
                ctx.drawImage(
                    image,
                    TileMapping[Tile["clicked"]][0] * tSize[0],
                    TileMapping[Tile["clicked"]][1] * tSize[1],
                    tSize[0],
                    tSize[1],
                ...
                ctx.drawImage(
                    image,
                    TileMapping[tile][0] * tSize[0],
                    TileMapping[tile][1] * tSize[1],
                    tSize[0],
                    tSize[1],
                    i * tSize[0] + boardOffset[0],
                    j * tSize[0] + boardOffset[1],
                ...
                if (gameBoardPancake[i][j]) {
                    ctx.drawImage(
                        image,
                        TileMapping[Tile["pancake"]][0] * tSize[0],
                        TileMapping[Tile["pancake"]][1] * tSize[1],
                        tSize[0],
                        tSize[1],
                        i * tSize[0] + boardOffset[0],
                        j * tSize[0] + boardOffset[1],
...
<Toaster></Toaster>
<div
    class="flex flex-col gap-4 w-min m-4 no-antialias mx-auto max-w-full px-4"
    bind:this={container}
>
    <div
    ...
    <div class="flex gap-2 justify-center items-center">
        <label>
            Presets
            <select
                class="min-w-[48px] w-full p-2 border-1"
                bind:value={selectedPreset}
                disabled={gameState.haveClickFirst}
            >
        ...
        <label>
            ...
            <input
                type="number"
                class="min-w-[48px] w-full p-2"
                bind:value={settings.w}
    ...
    >
        ...
        >
            <canvas
                bind:this={resetBtnCanvas}
                width={nikoMetadata.tileSize[0]}
                height={nikoMetadata.tileSize[1]}
            ></canvas>
        </button>
    ...
    <div class="flex gap-4 flex-wrap">
        <button
            ...
            }}>Mobile mode: {mobileMode ? "On" : "Off"}</button
        >
        <button
            class="btn grow"
            onclick={() => {
...
{#if mobileActionShow && mobileMode}
    ...
    >
        <div
            class="border-4 border-(--theme-color) p-4 bg-black flex gap-4 max-w-[480px] sm:mx-8 my-auto overflow-auto max-h-screen w-full flex-col"
        >
            <h1 class="h1-txt-size">Select an action</h1>
            <div class="flex gap-4 flex-wrap">
                <button
                    class="btn grow flex justify-center items-center"
                    aria-label="Dig"
                    bind:this={mobileActionDig}
                >
                    <div
                        class="sprite"
                        style="--action-image: url({Tiles}); background-position: -256px -64px"
                    ></div>
                </button>
                <button
                    class="btn grow flex justify-center items-center"
                    aria-label="Mark"
                    bind:this={mobileActionMark}
                >
                    <div
                        class="sprite"
                        style="--action-image: url({Tiles}); background-position: -192px -0px"
                    ></div>
                </button>
            </div>
        </div>
