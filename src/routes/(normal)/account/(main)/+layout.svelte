    ...
    >
        <div class="flex flex-col gap-2">
            <div class="flex lg:flex-col gap-4 lg:gap-2">
                ...
                >
                    <input
                        ...
                        onchange={async (event_trg) => {
                            if (event_trg.target) {
                                const inputElm = event_trg.target as HTMLInputElement;
                                ...
                                toast.promise(fetchReq, {
                                    ...
                                    error: (e) => `Failed to update profile picture: ${e.error}`,
                                });
                            }
                        }}
                    />
                ...
                <h1 class="h2-txt-size flex flex-row gap-4 items-center break-all">
                    {data.currentUser?.username}
                </h1>
            </div>
            <Link href={resolve("/(normal)/account/(main)")}>Personal Info</Link>
            <Link href={resolve("/(normal)/account/(main)/noik")}>Nikos :3</Link>
            <Link href={resolve("/(normal)/account/(main)/submissions")}>Submissions</Link>
            <Link href={resolve("/(normal)/account/(main)/posts")}>Posts</Link>
            <Link href={resolve("/(normal)/account/(main)/comments")}>Comments</Link>
            <Link href={resolve("/(normal)/account/(main)/migrate")}>Migrate Nikos</Link>
        </div>
        <div class="lg:px-4">
            {@render children?.()}
        </div>
