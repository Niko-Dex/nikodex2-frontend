<script lang="ts">
    ...
    onMount(async () => {
        ...
        await fetch(`/api/posts/user?user_id=${data.requestedInformation.id}`).then(async (e) => {
            if (e.ok) {
...
<section class="w-full relative flex justify-center p-4">
    ...
    <div class="max-w-[1200px] w-[1200px] flex flex-col gap-4 min-h-screen">
        ...
        <div class="h1-txt-size text-center flex flex-row gap-4 items-center p-2">
            <img
        ...
        {#if data.requestedInformation.account_type == AccountType.ADMIN}
