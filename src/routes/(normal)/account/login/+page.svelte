...
<section class="w-full relative flex justify-center" >
    ...
    <form onsubmit={() => {toast.loading(LOADING_TEXT)}} method="POST" class="flex flex-col max-w-[1200px] w-[1200px] p-4 gap-4 min-h-screen" action="?/loginRequest">
        <div class="flex flex-col items-center gap-0 m-4">
            <img src={Logo} style="--shadow: 5px 5px red" class="scale-80 drop-shadow-(--shadow)" alt="Nikodex Logo"/>
            <h1 class="h2-txt-size">Login</h1>
        </div>
        <label for="username-box">Username:</label>
        <input type="text" class="border-4 w-full" id="username" name="username" placeholder="Your username??"/>
        <label for="password-box">Password:</label>
        <input type="password" class="border-4 w-full" id="password" name="password" placeholder="Your password!! ssh... keep it a secret :3 also, pls use something different than 123"/>
        ...
        <button disabled={isProcessing} formaction="?/signUpRequest" type="submit" class="btn">Sign Up (doesn't work yet)</button>
    </form>
</section>