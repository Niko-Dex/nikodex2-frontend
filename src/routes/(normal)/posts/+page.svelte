<script lang="ts">
  let { form } = $props();
  import Background from "$lib/assets/images/page/posts/courtyard.png";
    ...
    >
      <div class="flex flex-col gap-2 w-full">
        ...
        >
          ...
          <label>
            <p>Content</p>
            <textarea name="content" class="w-full" bind:value={postContent}
            ></textarea>
          </label>
          <label>
            <p>Image</p>
...
<section class="w-full relative flex justify-center p-4">
  ...
  <div class="max-w-[1200px] w-[1200px] flex flex-col gap-4 min-h-screen">
    <div class="flex flex-row justify-between items-center">
      ...
      {#if currentUser}
        <button class="btn" onclick={() => openModal()}>Make a Post!</button>
      {:else}
    ...
    <p>
      <em
        >wanna share something? this is the place for you! just don't shitpost
        here :c</em
      >
    </p>
    {#if dataLoaded}
