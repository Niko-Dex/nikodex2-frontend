<script lang="ts">
  ...
  import DownloadIcon from "$lib/assets/images/components/DownloadIcon.svelte";
  let { data }: PageProps = $props();
  const imgPath = `/api/embed/nikosonas/${data.noikData.id}`;
...
<section class="w-full relative flex flex-col items-center p-4">
  ...
  >
    ...
    <div class="flex flex-col md:flex-row flex-wrap h-full gap-4 overflow-auto">
      <NikoImage
