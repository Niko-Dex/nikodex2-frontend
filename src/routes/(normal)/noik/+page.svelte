<script lang="ts">
    ...
    async function getData() {
        ...
        apiData = []
        try {
            const res = await fetch(`/api/data/page?page=${currentPage}&sort_by=${orderingOpt}`)
...
{#if !dataErr}
    <div class="bg-gray-700 flex justify-center gap-4 p-2 {pageSelectAtBottom ? "w-full" : "p-4 w-fit mx-auto sticky bottom-4"}">
        ...
        onclick={prevPage} disabled={currentPage <= 1 || !dataLoaded}>Prev</button>
        <input class="text-center border-2 border-white min-w-20"
        type="number" min={1} max={maxPages} bind:value={currentPage} aria-label="Page number" onchange={async () => await getData()} disabled={!dataLoaded}>
        <button 
        class="transition duration-100 hover:bg-white hover:text-black hover:cursor-pointer border-2 border-white px-4 py-1 disabled:opacity-50 disabled:pointer-events-none"
        onclick={nextPage} disabled={currentPage >= maxPages || !dataLoaded}>Next</button>
    </div>
{/if}