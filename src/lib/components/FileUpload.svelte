<script lang="ts">
    ...
    let { elm = $bindable(), accept, disabled, name, required }: Props = $props()
...
<div class="flex gap-2 fill-(--theme-color)">
    <label class="transition duration-200 border-4 border-(--theme-color) p-2 bg-black block w-full min-w-[300px] hover:bg-(--theme-color) hover:text-black hover:fill-black {disabled ? "pointer-events-none cursor-default" : "hover:cursor-pointer"}">
        <input type="file" bind:this={elm} class="hidden" accept={accept} disabled={disabled} name={name} bind:files={files} required={required}>
    ...
    <button class="hover:cursor-pointer btn p-2 hover:bg-(--theme-color) hover:fill-black" disabled={disabled} onclick={() => { if (elm) {elm.value = ""; files = null} }} aria-label="Clear">
