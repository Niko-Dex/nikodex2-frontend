...
<style>
    ...
    @keyframes square {
        0%, 24.9% { background-position-x: 0px; }
        25%, 49.9% { background-position-x: -192px; }
        50%, 74.5% { background-position-x: -384px; }
        75%, 99.9% { background-position-x: -576px; }
...
<div class="{expanded ? "transition duration-200 fixed w-screen h-screen top-0 left-0 z-5 bg-black/75 flex justify-center" : "w-full"} card">
    <div class="border-4 border-(--theme-color) p-4 bg-black flex gap-4 {expanded ? "max-w-[1200px] mx-8 my-auto overflow-auto max-h-screen" : "max-w-full"} {!expanded && !full_expand ? "lg:max-w-[640px]" : ""} w-full flex-col md:flex-row">
        <div class="img flex flex-col gap-2 bg-black {expanded ? "sticky top-0 z-1" : ""}">
            <button class="max-w-[256px] w-full md:w-[256px] max-h-[256px] h-fit hover:cursor-grab {isPatPat ? "pointer-events-none" : ""}" onpointerdown={patpat}>
                {#if isBlacklisted}
                <div class="relative w-full h-full">
                    ...
                    <div class="{isPatPat ? "opacity-100" : "opacity-0"}">
                        <div class="{isPatPat ? "opacity-0" : "opacity-100"} transition duration-1000 absolute w-full h-full bg-(image:--bg) top-0 left-0 z-1 anim" style="--bg: url({Squares}); transition-timing-function: cubic-bezier(0.260, 0.010, 0.310, 0.990);"></div>
                    </div>
                </div>
                {:else}
