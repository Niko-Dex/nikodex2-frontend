...
{#if deleteRequested}
  <div class="fixed flex left-0 top-0 w-screen h-screen bg-black/60 z-20 p-4">
    <section
      class="min-w-32 min-h-32 text-center items-center flex md:flex-row flex-col gap-4 bg-black p-4 border-4 m-auto border-(--theme-color)"
    >
      <div
        class="relative"
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="grid md:grid-cols-2 gap-4 mt-auto">
          ...
          >
          <button class="btn" onclick={() => (deleteRequested = false)}
            >No</button
          >
        </div>
      </div>
