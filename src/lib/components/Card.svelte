...
<div class={expanded ? "transition duration-200 fixed w-screen h-screen top-0 left-0 z-5 bg-black/75 flex justify-center items-center" : "w-full"}>
    <div class="border-4 border-amber-600 p-4 bg-black flex gap-4 {expanded ? "max-w-[1200px] m-8" : "max-w-full lg:max-w-[640px]"} w-full flex-col md:flex-row rounded-md">
        <div class="img flex flex-col gap-2">
            <img src={img_link} alt="nikosona of {name} by {author}" class="no-antialias max-w-[256px] h-fit">
