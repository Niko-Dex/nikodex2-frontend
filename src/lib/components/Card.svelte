<script lang="ts">
    ...
    function patpat(ev: MouseEvent & { currentTarget: EventTarget & HTMLButtonElement; }) {
        const absolute = new URL(imgElm.src, window.location.origin).href;
        imgElm.src = patpatAPI + absolute
    }
    ...
    <div class="border-4 border-amber-600 p-4 bg-black flex gap-4 {expanded ? "max-w-[1200px] m-8" : "max-w-full lg:max-w-[640px]"} w-full flex-col md:flex-row">
        <div class="img flex flex-col gap-2">
            <button class="max-w-[256px] h-fit hover:cursor-grab" onclick={patpat}>
                <img src={img_link} alt="nikosona of {name} by {author}" class="no-antialias max-w-[256px] h-fit" bind:this={imgElm}>
            </button>
