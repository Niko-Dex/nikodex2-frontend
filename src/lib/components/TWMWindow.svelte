...
{#if visible}
<div class="twm-window fixed top-0 left-0 z-15 bg-black border-4 border-(--theme-color) min-w-[320px] min-h-[240px] grid grid-rows-[48px_1fr]" bind:this={dialog}>
    <div class="border-b-4 border-(--theme-color) flex items-center p-2 justify-between hover:cursor-grab" onpointerdown={mouseDown} role="dialog" tabindex="-1">
        <p class="select-none">{title}</p>
        <button class="bg-(--theme-color) hover:cursor-pointer hover:bg-white transition duration-100" aria-label="Close" onclick={() => visible = false}>
